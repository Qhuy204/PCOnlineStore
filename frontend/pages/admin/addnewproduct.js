...
const AddNewProduct = () => {
    ...
    // State cho giá trị biến thể đã chọn
    ...
    useEffect(() => {
        if (product.category_id) {
            ...
            if (selectedCategory && selectedCategory.attributes) {
                ...
                // Thiết lập thuộc tính cho thông số kỹ thuật
                ...
                // Thiết lập thuộc tính cho biến thể
                ...
                if (existingValue) {
                    ...
                    if (updatedCategory) {
                        ...
                        const updatedSelectedAttributes = updatedCategory.attributes.filter(
                            attr => selectedVariantAttributes.some(
    ...
    const handleAddUrlImage = () => {
        if (urlInput.trim() !== '') {
            // Sử dụng URL cố định thay vì Blob URL
    ...
    const handleDeviceUpload = async (e) => {
        try {
            ...
            const uploadPromises = e.files.map(async (file, index) => {
                try {
            ...
            if (validImages.length === 0) {
                toast.current.show({
                    severity: 'error',
                    summary: 'Lỗi',
                    detail: 'Không có ảnh nào được tải lên thành công'
                });
                return;
            }
            
            if (validImages.length < e.files.length) {
                toast.current.show({
                    severity: 'warn',
                    summary: 'Cảnh báo',
                    detail: `Chỉ tải lên thành công ${validImages.length}/${e.files.length} ảnh`
                });
            }
    ...
    const handleVariantAttributeSelect = (e) => {
        ...
        const selectedAttrs = variantAttributeTypes.filter(
            attr => selectedAttributeIds.includes(attr.attribute_type_id)
        );
        
        // Lọc ra các thuộc tính đã bị loại bỏ
        ...
        // Xóa giá trị đã chọn của các thuộc tính đã bị loại bỏ
    ...
    // Kiểm tra xem giá trị có tồn tại chưa
    const checkAttributeValueExists = async (attribute_type_id, value_name) => {
        try {
            // Kiểm tra trong dữ liệu hiện tại trước
            ...
            if (existingAttr && existingAttr.values) {
                const existingValue = existingAttr.values.find(
                    v => v.value_name.toLowerCase() === value_name.toLowerCase()
                );
                if (existingValue) {
    ...
    const handleAddAttributeValue = async () => {
        if (newAttributeValue.trim() && currentAttributeType) {
            try {
                ...
                if (existingValue) {
                    // Nếu giá trị đã tồn tại, thông báo và thêm vào danh sách đã chọn
                    toast.current.show({
                        severity: 'info',
                        summary: 'Thông báo',
                        detail: 'Giá trị này đã tồn tại, đã được thêm vào danh sách'
                    ...
                    // Tự động chọn giá trị đã tồn tại
                    setSelectedVariantValues(prev => {
                ...
                // Cập nhật categoryAttributes
                const updatedCategory = categoriesData.find(cat => cat.category_id === product.category_id);
                if (updatedCategory) {
                    setCategoryAttributes(updatedCategory.attributes);
                    
                    // Cập nhật variantAttributeTypes
                    setVariantAttributeTypes(updatedCategory.attributes);
                    
                    // Cập nhật selectedVariantAttributes
                    const updatedSelectedAttributes = updatedCategory.attributes.filter(
                        attr => selectedVariantAttributes.some(
                            selectedAttr => selectedAttr.attribute_type_id === attr.attribute_type_id
                        )
                ...
                // Tự động chọn giá trị mới thêm vào
                setSelectedVariantValues(prev => {
                    const currentValues = prev[currentAttributeType.attribute_type_id] || [];
                    return {
                        ...prev,
            ...
            try {
                ...
                for (const value of attributeInput.values) {
                    try {
                        ...
                        console.error("Error adding attribute value:", error);
                        // Tiếp tục với giá trị tiếp theo nếu có lỗi
                    }
                }
                ...
                if (product.category_id) {
                    ...
                    if (updatedCategory) {
                        ...
                        // Tự động chọn thuộc tính mới thêm vào
                        ...
                        if (newAttributeInCategory) {
                            ...
                            // Chọn tất cả giá trị của thuộc tính mới
    ...
    const validateProduct = () => {
        ...
        // Kiểm tra thông số kỹ thuật bắt buộc
        if (product.category_id) {
    ...
    // Hàm lấy product_id mới nhất sau khi thêm sản phẩm
    const getLatestProductId = async () => {
        try {
            const products = await productsService.getAll();
            if (products && products.length > 0) {
                // Tìm product_id lớn nhất
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            let productId = productResponse.product_id;
          
            // Nếu không nhận được product_id từ response, lấy ID mới nhất
            if (!productId) {
            ...
            // 2. Save product specifications với product_id đã có
            ...
            toast.current.show({ severity: 'success', summary: 'Thành công', detail: 'Sản phẩm đã được lưu thành công' });
            ...
            toast.current.show({ 
                severity: 'error', 
                summary: 'Lỗi', 
                detail: 'Không thể lưu sản phẩm: ' + error.message 
            });
    ...
    // Handle chọn giá trị cho thuộc tính biến thể
    ...
    // Kiểm tra xem thuộc tính và giá trị đã chọn đủ để tạo biến thể chưa
    const canGenerateVariants = () => {
        if (selectedVariantAttributes.length === 0) return false;
        
        // Kiểm tra mỗi thuộc tính đã chọn phải có ít nhất 1 giá trị
...
const handleShowVariantPreview = () => {
    ...
    // Lấy các giá trị đã chọn cho mỗi thuộc tính
    ...
    // Kiểm tra nếu có thuộc tính không có giá trị nào
    ...
    const generateCombinations = (lists, current = [], index = 0) => {
        ...
        for (const item of lists[index].values) {
            results.push(...generateCombinations(
                lists,
                [...current, { 
    ...
    // Tạo danh sách biến thể xem trước
    const variantsList = combinations.map((combo, index) => {
        // Tạo tên biến thể với định dạng: Tên sản phẩm/Giá trị 1/Giá trị 2/...
        const variantName = product.product_name + '/' + combo.map(item => item.valueName).join('/');
        
        // Tạo object chứa thông tin về các thuộc tính
        const attributes = {};
        combo.forEach(item => {
    ...
    setSelectedPreviewVariants(variantsList); // Mặc định chọn tất cả
    setShowVariantPreview(true);
};

// Tạo biến thể từ danh sách đã chọn
const generateVariants = () => {
    ...
    const variants = selectedPreviewVariants.map((variant, index) => {
        // Tạo SKU từ các thuộc tính đã chọn
        ...
        const skuParts = [
        removeVietnameseTones(product.brand_name).toUpperCase().replace(/\s+/g, ''), // ASUS
        removeVietnameseTones(product.product_name.split(' ')[2] || '').toUpperCase(), // ROG, Strix,...
        ...variant.attributes.map(attr => 
            removeVietnameseTones(attr.valueName)
            .toUpperCase()
            .replace(/\s+/g, '-')
        )
        ];
        ...
        return {
            ...
            // Thêm trường chỉ đọc để hiển thị thông tin
            readOnlyInfo: {
                priceAdjustment: 0,
                stockQuantity: 0
            }
        ...
        <div className="flex flex-wrap gap-2 mt-2">
            {selectedValueNames.map((name, index) => (
                <Chip 
                    ...
                    removable 
                    onRemove={() => {
                        const newSelectedIds = selectedIds.filter((id, idx) => 
                            attribute.values[idx] && attribute.values[idx].value_name !== name
                        );
...
return (
    <div className="add-product-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
        ...
        <div className="grid" style={{justifyContent: 'center', width: '100%'}}>
            <div className="col-12 lg:col-8" style={{width: '100%'}}>
                <Card className="mb-3">
                    ...
                    <div className="field">
                        <label htmlFor="product_name" className="font-bold block mb-2">Tên sản phẩm <span className="text-red-500">*</span></label>
                        <InputText 
                            id="product_name" 
                    ...
                    <div className="grid">
                        <div className="col-12 md:col-6">
                            <div className="field">
                                <label htmlFor="brand_id" className="font-bold block mb-2">Thương hiệu <span className="text-red-500">*</span></label>
                                <Dropdown
                                    id="brand_id"
                        ...
                        <div className="col-12 md:col-6">
                            <div className="field">
                                <label htmlFor="model" className="font-bold block mb-2">Model</label>
                                <InputText 
                                    id="model" 
                                    value={product.model} 
                    ...
                    <div className="field">
                        <label htmlFor="category_id" className="font-bold block mb-2">Loại sản phẩm <span className="text-red-500">*</span></label>
                        <Dropdown 
                            id="category_id" 
                    ...
                    <div className="field">
                        <label htmlFor="description" className="font-bold block mb-2">Mô tả sản phẩm</label>
                        <ReactQuill
                    ...
                    <div className="grid">
                        <div className="col-12 md:col-6">
                            <div className="field">
                                <label htmlFor="base_price" className="font-bold block mb-2">Giá cơ bản <span className="text-red-500">*</span></label>
                                <InputNumber 
                                    id="base_price" 
                        ...
                        <div className="col-12 md:col-6">
                            <div className="field flex align-items-center mt-4">
                                <Checkbox 
                                    ...
                                    className="mr-2"
                                />
                                <label htmlFor="is_featured" className="font-bold cursor-pointer">Nổi bật</label>
                            </div>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="mb-3">
                    <div className="flex justify-content-between align-items-center">
                        ...
                        <i className="pi pi-question-circle image-help-icon" data-pr-tooltip="Thêm hình ảnh sản phẩm. Ảnh đầu tiên sẽ là ảnh đại diện"></i>
                    </div>
                    <div className="product-images-container grid">
                        {product.images.map((img, index) => (
                            <div key={index} className="col-6 md:col-4 lg:col-3 p-2">
                                <div className={`image-preview ${img.is_primary ? 'border-primary border-2' : 'border-1'} border-round p-1`} 
                                    onClick={() => handleSetPrimary(index)}
                                    style={{ position: 'relative', height: '150px', overflow: 'hidden' }}
                                >
                                    <img src={img.image_url} alt={`Product ${index}`} className="w-full h-full" style={{ objectFit: 'cover' }} />
                                    {img.is_primary && (
                        ...
                        <div className="col-6 md:col-4 lg:col-3 p-2">
                            <div className="image-upload-placeholder border-dashed border-round border-1 flex align-items-center justify-content-center flex-column" 
                        ...
                        <div className="mt-2 mb-4">
                            <Tag severity="info" icon="pi pi-info-circle">
                                Danh mục này yêu cầu các thông số kỹ thuật bắt buộc: {
                                    categoryAttributes.filter(attr => attr.is_required).map(attr => attr.type_name).join(', ')
                                }
                            </Tag>
                        </div>
                    )}
                ...
                <Card className="mb-3">
                    ...
                    <div className="field-checkbox mb-3">
                        ...
                        />
                        <label htmlFor="hasVariants" className="ml-2 cursor-pointer">Sản phẩm này có nhiều biến thể (khác nhau về kích thước, màu sắc, v.v.)</label>
                    </div>
                            ...
                            {product.category_id ? (
                                <>
                                    ...
                                    <Dialog 
                                        ...
                                        footer={
                                            <div>
                                                <Button label="Hủy" icon="pi pi-times" className="p-button-text" onClick={() => setShowVariantPreview(false)} />
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {selectedVariantAttributes.map(attr => (
                                                <Column key={attr.attribute_type_id} field={`attr_${attr.attribute_type_id}`} header={attr.type_name} />
                                            ))}
                                        </DataTable>
                                    </Dialog>
                                            ...
                                            >
                                                ...
                                                <Column 
                                                    ...
                                                    body={(rowData) => {
                                                        ...
                                                        return (
                                                            <span className={adjustment < 0 ? 'text-red-500' : (adjustment > 0 ? 'text-green-500' : '')}>
        ...
        <div className="fixed bottom-0 left-0 w-full p-3 flex justify-content-end">
            <Button label="Hủy" icon="pi pi-times" className="p-button-outlined p-button-secondary mr-2" />
            <Button label="Lưu" icon="pi pi-save" className="p-button-primary"  onClick={handleSave} loading={loading} />
        </div>

        {/* Dialog cho thêm ảnh từ URL */}
        <Dialog header="Thêm hình ảnh từ URL" visible={displayUrlDialog} style={{ width: '450px' }} onHide={() => setDisplayUrlDialog(false)}>
            <div className="field">
                <label htmlFor="imageUrl" className="font-bold block mb-2">URL hình ảnh</label>
                <InputText 
                    id="imageUrl" 
                    value={urlInput} 
            ...
            <div className="flex justify-content-end mt-4">
                <Button label="Hủy" icon="pi pi-times" className="p-button-text mr-2" onClick={() => setDisplayUrlDialog(false)} />
                <Button label="Thêm" icon="pi pi-check" className="p-button-text" onClick={handleAddUrlImage} />
            </div>
        </Dialog>

        {/* Dialog cho thêm ảnh từ thiết bị */}
        <Dialog header="Thêm hình ảnh từ thiết bị" visible={displayDeviceDialog} style={{ width: '500px' }} onHide={() => setDisplayDeviceDialog(false)}>
            <FileUpload 
                name="files" 
                customUpload 
            ...
            />
            <div className="flex justify-content-end mt-4">
                <Button label="Đóng" icon="pi pi-times" className="p-button-text" onClick={() => setDisplayDeviceDialog(false)} />
            </div>
        </Dialog>

        {/* Dialog cho thêm thông số kỹ thuật */}
        <Dialog header="Thêm thông số kỹ thuật" visible={displaySpecDialog} style={{ width: '450px' }} onHide={() => setDisplaySpecDialog(false)}>
                <div className="field mb-3">
                    <label htmlFor="spec_name" className="font-bold block mb-2">Tên thông số</label>
                ...
                <div className="field mb-3">
                    <label htmlFor="spec_value" className="font-bold block mb-2">Giá trị</label>
                ...
                <div className="field">
                    <label htmlFor="display_order" className="font-bold block mb-2">Thứ tự hiển thị</label>
                    <InputNumber 
                        id="display_order" 
                        value={specInput.display_order} 
                        onValueChange={(e) => setSpecInput(prev => ({ ...prev, display_order: e.value }))}
                        className="w-full"
                        min={0}
                    />
                </div>
                <div className="flex justify-content-end mt-4">
                    <Button label="Hủy" icon="pi pi-times" className="p-button-text mr-2" onClick={() => setDisplaySpecDialog(false)} />
            ...
            <Dialog header="Thêm thuộc tính biến thể" visible={displayAttributeDialog} style={{ width: '500px' }} onHide={() => setDisplayAttributeDialog(false)}>
                <div className="field mb-3">
                    <label htmlFor="type_name" className="font-bold block mb-2">Tên thuộc tính</label>
                    <InputText 
                        id="type_name" 
                ...
                <div className="field mb-3">
                    ...
                    <InputNumber 
                        ...
                        onValueChange={(e) => setAttributeInput(prev => ({ ...prev, display_order: e.value }))}
                        className="w-full"
                        min={0}
                    />
                </div>
                ...
                <div className="flex justify-content-end mt-4">
                    <Button label="Hủy" icon="pi pi-times" className="p-button-text mr-2" onClick={() => setDisplayAttributeDialog(false)} />
                    <Button label="Lưu" icon="pi pi-check" className="p-button-text" onClick={handleSaveAttribute} loading={loading} />
                </div>
            </Dialog>

            {/* Dialog cho thêm giá trị thuộc tính vào một thuộc tính đã có */}
            <Dialog header={`Thêm giá trị cho ${currentAttributeType?.type_name || 'thuộc tính'}`} visible={displayAttributeValueDialog} style={{ width: '450px' }} onHide={() => setDisplayAttributeValueDialog(false)}>
                <div className="field mb-3">
                        ...
                        <div className="border-1 border-round p-3">
                            <div className="flex flex-wrap gap-2">
                                {currentAttributeType.values.map(value => (
                                    <Tag key={value.attribute_value_id} 
                ...
                <div className="flex justify-content-end mt-4">
                    <Button label="Đóng" icon="pi pi-times" className="p-button-text" onClick={() => setDisplayAttributeValueDialog(false)} />
                </div>
            </Dialog>

            {/* Dialog cho chọn ảnh cho biến thể */}
            <Dialog header="Chọn ảnh cho biến thể" visible={displayVariantImageDialog} style={{ width: '600px' }} onHide={() => setDisplayVariantImageDialog(false)}>
                ...
                <div className="flex justify-content-end mt-4">
                    <Button label="Đóng" icon="pi pi-times" className="p-button-text" onClick={() => setDisplayVariantImageDialog(false)} />
                </div>
            </Dialog>
        </div>
    );
