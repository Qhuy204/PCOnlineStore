...
const EditProductPage = () => {
  ...
  const { product_id: productId } = router.query
  ...
  const handleDeviceUpload = (e) => {
    // We'll just store the file paths in a format suitable for your database
    const uploadedImages = e.files.map((file, index) => {
      // Create a path for the image that matches your expected format
      // This is just for display until the actual save happens
      const fileName = file.name;
      const imagePath = `/img_data/${determineCategory()}/${fileName}`;
      
      return {
        image_url: imagePath,
        is_primary: product.images.length === 0 && index === 0,
        file: file, // Store the file object for later use in the actual update function
      };
    });
  ...
  // Helper function to determine folder based on category
  const determineCategory = () => {
    const categoryName = product.category_name || '';
    const category = categoryName.toLowerCase();
    
    if (category.includes('laptop')) return 'laptop';
    if (category.includes('máy tính bàn')) return 'pc';
    if (category.includes('màn hình')) return 'monitor';
    if (category.includes('bàn phím')) return 'keyboard';
    if (category.includes('chuột')) return 'mouse';
    if (category.includes('tai nghe')) return 'earphone';
    if (category.includes('loa')) return 'loudspeaker';
    if (category.includes('ổ cứng')) return 'ssd';
    if (category.includes('ram')) return 'ram';
    if (category.includes('card đồ họa')) return 'vga';
    if (category.includes('mainboard')) return 'mainboard';
    if (category.includes('cpu')) return 'cpu';
    if (category.includes('nguồn')) return 'psu';
    if (category.includes('tản nhiệt')) return 'accessory';
  ...
  // Fetch product data
// Modify the fetchProductData function to add inline error handling for 404 errors

const fetchProductData = async () => {
    try {
      // Tải thông tin cơ bản của sản phẩm
      const basicProductInfo = await productsService.getById(productId);
  
      if (!basicProductInfo) {
        throw new Error("Không tìm thấy sản phẩm");
      }
  
      // Nếu brand_name chưa có, lấy thông tin thương hiệu từ brandService
      let brand_name = basicProductInfo.brand_name;
      if (!brand_name && basicProductInfo.brand_id) {
        const brandData = await brandService.getById(basicProductInfo.brand_id);
        brand_name = brandData ? brandData.brand_name : "";
      }
  
      // Tải thông số kỹ thuật, hình ảnh, biến thể
      let specifications = [];
      let images = [];
      let variants = [];
      ...
      try {
        specifications = await Product_SpecificationsService.getById(productId);
      } catch (error) {
        if (error.response && error.response.status === 404) {
          console.log("Không tìm thấy thông số kỹ thuật cho sản phẩm này");
          specifications = [];
        } else {
        //   throw error;
        }
      ...
      // Tải hình ảnh
      try {
        images = await Product_ImagesService.getById(productId);
      } catch (error) {
        if (error.response && error.response.status === 404) {
          console.log("Không tìm thấy hình ảnh cho sản phẩm này");
          images = [];
        } else {
        //   throw error;
        }
      ...
      // Tải biến thể
      try {
        variants = await Product_VariantsService.getById(productId);
      } catch (error) {
        if (error.response && error.response.status === 404) {
          console.log("Không tìm thấy biến thể cho sản phẩm này");
          variants = [];
        } else {
        //   throw error;
        }
      ...
      // Fix for HTML description
      let description = basicProductInfo.description ? String(basicProductInfo.description) : "";
      console.log('desc:', description);
      try {
        if (description.includes("&lt;") || description.includes("&gt;") || description.includes("&quot;")) {
          const parser = new DOMParser();
          const parsedDoc = parser.parseFromString(description, 'text/html');
          description = parsedDoc.body.innerHTML;
        }
      } catch (error) {
      ...
      setProduct({
        ...basicProductInfo,
        brand_name,
        category_name: basicProductInfo.category_name,
      ...
      if (hasVariants && variants.length > 0) {
        const processedVariants = await Promise.all(
          variants.map(async (variant) => {
            try {
              const variantAttributeValues = await Variant_Attribute_ValuesService.getById(variant.variant_id);
              return {
                ...variant,
              ...
              if (error.response && error.response.status === 404) {
                console.log(`Không tìm thấy giá trị thuộc tính cho biến thể ${variant.variant_id}`);
                return {
                  ...variant,
                  attributes: [],
                };
        ...
        if (processedVariants.length > 0 && processedVariants[0].attributes && processedVariants[0].attributes.length > 0) {
          ...
          if (basicProductInfo.category_id) {
      ...
      // Xử lý lỗi 404 cho sản phẩm
      if (error.response && error.response.status === 404) {
        toast.current?.show({
          severity: 'info',
          summary: 'Thông báo',
          detail: 'Không tìm thấy sản phẩm này',
          life: 3000
        });
      } else {
  ...
  useEffect(() => {
    if (product.brand_name && brandOptions.length > 0) {
      const matchingBrand = brandOptions.find(
        (brand) => brand.name && brand.name.toLowerCase() === product.brand_name.toLowerCase()
      );
      if (matchingBrand) {
        setProduct((prev) => ({
          ...prev,
          brand_id: matchingBrand.value,
        }));
      }
    }
  }, [product.brand_name, brandOptions]);
  
  useEffect(() => {
    if (product.category_name && categoryOptions.length > 0) {
      const matchingCategory = categoryOptions.find(
        (cat) => cat.label && cat.label.toLowerCase() === product.category_name.toLowerCase()
      );
      if (matchingCategory) {
        setProduct((prev) => ({
          ...prev,
          category_id: matchingCategory.value,
        }));
      }
    }
  }, [product.category_name, categoryOptions]);
  ...
  useEffect(() => {
    if (product.category_id) {
      ...
      if (selectedCategory && selectedCategory.attributes) {
        ...
        if (product.product_id && selectedVariantAttributes.length === 0) {
  ...
  }, [product.category_id, categories, product.product_id, selectedVariantAttributes.length])
  ...
  // Các hàm xử lý variant, hình ảnh, thuộc tính biến thể... (giữ nguyên code gốc)
  ...
  const handleRemoveVariant = async (variantId) => {
    try {
      ...
      if (!variantId.toString().startsWith('temp-')) {
        await Product_VariantsService.delete(variantId);
        toast.current?.show({
          severity: "success",
          summary: "Thành công",
          detail: "Đã xóa biến thể khỏi cơ sở dữ liệu",
        });
      }
      ...
      toast.current?.show({
        severity: "error",
        summary: "Lỗi",
        detail: "Không thể xóa biến thể: " + error.message,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleRemoveImage = async (index) => {
    try {
      ...
      if (imageToRemove.image_id) {
        setLoading(true);
        await Product_ImagesService.delete(imageToRemove.image_id);
        toast.current?.show({
          severity: "success",
          summary: "Thành công",
          detail: "Đã xóa hình ảnh khỏi cơ sở dữ liệu",
        });
      }
      ...
      // If we removed the primary image, set a new primary image
      ...
      toast.current?.show({
        severity: "error",
        summary: "Lỗi",
        detail: "Không thể xóa hình ảnh: " + error.message,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleRemoveSpecification = async (index) => {
    try {
      const specToRemove = product.specifications[index];
      if (specToRemove.spec_id) {
        setLoading(true);
        await Product_SpecificationsService.delete(specToRemove.spec_id);
        toast.current?.show({
          severity: "success",
          summary: "Thành công",
          detail: "Đã xóa thông số khỏi cơ sở dữ liệu",
        });
      }
      ...
      toast.current?.show({
        severity: "error",
        summary: "Lỗi",
        detail: "Không thể xóa thông số: " + error.message,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleAddNewVariantValue = async (attr) => {
    if (newVariantValue.trim()) {
      try {
        ...
        if (existingValue) {
          ...
          setSelectedVariantValues((prev) => {
            const currentValues = prev[attr.attribute_type_id] || [];
            if (!currentValues.includes(existingValue.attribute_value_id)) {
              return {
                ...prev,
                [attr.attribute_type_id]: [...currentValues, existingValue.attribute_value_id],
              };
            }
            return prev;
          });
        ...
        setSelectedVariantValues((prev) => {
          ...
          return {
            ...prev,
            [attr.attribute_type_id]: [...currentValues, responseData.attribute_value_id],
          };
        });
        
        setNewVariantValue("");
        toast.current?.show({ severity: "success", summary: "Thành công", detail: "Đã thêm giá trị thuộc tính mới" });
      } catch (error) {
        console.error("Error adding attribute value:", error);
        toast.current?.show({
          severity: "error",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (prodId) {
        ...
        prodId = productResponse.product_id;
        if (!prodId) {
          prodId = await getLatestProductId();
          if (!prodId) throw new Error("Không thể xác định ID sản phẩm");
        }
      }
      ...
      for (const spec of product.specifications) {
        if (spec.spec_id) {
          // Nếu có spec_id, cập nhật bản ghi cũ
          await Product_SpecificationsService.update({
      ...
      for (const img of product.images) {
        if (img.image_id) {
          await Product_ImagesService.update({
      ...
      // Cập nhật hoặc insert biến thể
      if (product.hasVariants && productVariants.length > 0) {
        for (const variant of productVariants) {
          let variantId;
          try {
            if (variant.variant_id && !variant.variant_id.toString().startsWith('temp-')) {
              // Nếu đã có ID và không phải ID tạm, update bản ghi cũ
              await Product_VariantsService.update({
              ...
              // Insert mới nếu không có ID hoặc có ID tạm
              const variantResponse = await Product_VariantsService.insert({
                product_id: prodId,
                variant_sku: variant.variant_sku,
                price_adjustment: variant.price_adjustment || 0,
                stock_quantity: variant.stock_quantity || 0,
              ...
              variantId = variantResponse.variant_id;
            }
  
            // Cập nhật giá trị thuộc tính cho biến thể:
            if (variant.attributes && Array.isArray(variant.attributes)) {
              for (const attr of variant.attributes) {
                ...
                // Với mỗi giá trị, nếu đã tồn tại (có id) thì update, ngược lại insert mới
                if (attr.va_id) {
                  await Variant_Attribute_ValuesService.update({
              ...
              console.log("Variant has no attributes or attributes is not an array:", variant);
            }
          } catch (error) {
            console.error("Error inserting/updating variant:", error);
      ...
      toast.current?.show({
        severity: "error",
        summary: "Lỗi",
        detail: "Không thể cập nhật sản phẩm: " + error.message,
      });
  ...
  const handleShowVariantPreview = () => {
    ...
    setSelectedPreviewVariants(variantsList)
  ...
  const generateVariants = () => {
    ...
    const variants = selectedPreviewVariants.map((variant, index) => {
      ...
      const skuParts = [
        removeVietnameseTones(product.brand_name).toUpperCase().replace(/\s+/g, ""),
        removeVietnameseTones(product.product_name.split(" ")[2] || "").toUpperCase(),
        ...variant.attributes.map((attr) =>
          removeVietnameseTones(attr.valueName).toUpperCase().replace(/\s+/g, "-")
        ),
      ]
      return {
  ...
  if (loading) {
    return (
      <div className="flex justify-content-center align-items-center" style={{ height: "70vh" }}>
        <ProgressSpinner />
        <span className="ml-3">Đang tải dữ liệu sản phẩm...</span>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="p-3">
        ...
        <div className="mt-3">
          <button className="p-button p-component" onClick={() => router.push("/admin/products")}>
  ...
  return (
    <div className="edit-product-page p-3">
      <Toast ref={toast} />
        <div className="product-form-container" style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <div className="grid" style={{ justifyContent: "center", width: "100%" }}>
            <div className="col-12 lg:col-8" style={{ width: "100%" }}>
            <div className="flex align-items-center justify-content-between mb-3">
                <h2 className="m-0">Cập nhật sản phẩm: {product.product_name}</h2>
            </div>

            <p className="text-500 mb-4">Chỉnh sửa thông tin sản phẩm. Các trường có dấu (*) là bắt buộc.</p>
              <Card className="mb-3">
                  ...
                  <DataTable value={product.specifications} className="p-datatable-sm mt-3" stripedRows>
                    ...
                    <Column
                      body={(rowData, options) => (
                        ...
                        onClick={() => handleRemoveSpecification(options.rowIndex)}
                        loading={loading}
                        />
                      )}
                              ...
                              <Column
                                ...
                                body={(rowData) => {
                                  ...
                                  return (
                                    <span
                                      className={
                                        adjustment < 0 ? "text-red-500" : adjustment > 0 ? "text-green-500" : ""
                                      }
                                    >
