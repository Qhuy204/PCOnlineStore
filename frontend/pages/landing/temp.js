...
const EditProductPage = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const specifications = await Product_SpecificationsService.getById(productId)
      const images = await Product_ImagesService.getById(productId)
      const variants = await Product_VariantsService.getById(productId)
      ...
      // Nếu có biến thể, xử lý các thuộc tính của biến thể
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (prodId) {
        // Update existing product - Đảm bảo prodId là số
        console.log("Updating product with ID:", prodId);
        await productsService.update(productData)
        // Cập nhật thêm thông số mới
        await Product_SpecificationsService.update(spec_id);
        await Product_ImagesService.update(image_id);
      } else {
        // Create new product
